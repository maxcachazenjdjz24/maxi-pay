...
// ─── Pending Approvals (gasto/replicación que requieren aprobación humana) ───
...
export function insertPendingApproval(
  ...
  db.prepare(
    `INSERT INTO pending_approvals (id, kind, amount_cents, recipient, reason, metadata, status, created_at)
     VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)`,
  ).run(
    row.id,
    row.kind,
...
export function getPendingApprovals(db: DatabaseType): PendingApprovalRow[] {
  const rows = db
    .prepare("SELECT * FROM pending_approvals WHERE status = 'pending' ORDER BY created_at ASC")
    .all() as any[];
  return rows.map(deserializePendingApproval);
}

export function getPendingApprovalById(db: DatabaseType, id: string): PendingApprovalRow | undefined {
  const row = db
    .prepare("SELECT * FROM pending_approvals WHERE id = ?")
    .get(id) as any | undefined;
  return row ? deserializePendingApproval(row) : undefined;
}
...
export function resolvePendingApproval(
  ...
  db.prepare(
    `UPDATE pending_approvals SET status = ?, resolved_at = datetime('now'), resolved_by = ? WHERE id = ?`,
  ).run(status, resolvedBy, id);
}

function deserializePendingApproval(row: any): PendingApprovalRow {
  return {
    id: row.id,
    kind: row.kind,
...
// (reservado: Inbox State Machine Helpers eliminados — mensajería entre
// agentes Conway, no aplica a esta versión)
...
// (reservado: deserializeRegistry, deserializeInboxMessage, deserializeReputation
// eliminados — no aplican a esta versión)
...
// (reservado: Agent Cache y Onchain Transaction DB Helpers eliminados —
// descubrimiento on-chain de Conway, no aplica a esta versión)
