...
export const MIGRATION_V3 = `
  -- (reservado: inbox_messages entre agentes Conway eliminado — no aplica a esta versión)
`;
...
// (reservado: ALTERs de inbox_messages eliminados — tabla no aplica a esta versión)
...
export const MIGRATION_V7 = `
  ...
  -- (reservado: discovered_agents_cache y onchain_transactions eliminadas —
  -- específicas del descubrimiento de agentes on-chain de Conway, no aplican aquí)
`;
...
// === Schema version: 12 ===
// Nueva (no existía en el original): cola de aprobaciones humanas para
// gastos por encima de requireConfirmationAboveCents, y para
// replicaciones (spawn de hijos) que alcanzaron la condición 2x/3x.
export const MIGRATION_V12 = `
  CREATE TABLE IF NOT EXISTS pending_approvals (
    id TEXT PRIMARY KEY,                    -- ULID
    kind TEXT NOT NULL CHECK(kind IN ('spend', 'replication')),
    amount_cents INTEGER,                   -- para kind='spend'
    recipient TEXT,                         -- para kind='spend'
    reason TEXT NOT NULL,
    metadata TEXT NOT NULL DEFAULT '{}',    -- JSON: detalles adicionales
    status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending', 'approved', 'denied', 'expired')),
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    resolved_at TEXT,
    resolved_by TEXT                        -- operatorId de quien decidió
  );
